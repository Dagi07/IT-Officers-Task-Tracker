...
async function getAlertAmount() {
  try {
    ...
    let fetchSQL = `SELECT COUNT(tomorrow_id) FROM tomorrow_table WHERE registered_date>'${yesterday} 00:00:00' AND  registered_date<'${today} 00:00:00'`;
    // Execute the query (use the query method from the db connection file)
    let result = await conn.query(fetchSQL);
    ...
    console.error("Error in fetching for today", err);
  }
}

module.exports = { fetchForToday, getAlertAmount };
